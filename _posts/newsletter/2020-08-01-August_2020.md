...
In [simple terms](https://medium.com/@nopara73/what-is-an-xpub-4b60e63ec22d), while anyone who has access to your xPub alone *cannot* spend your bitcoin funds, they *can* see all addresses derived from it, all the coins you control at those addresses, and any transaction history. This is why the label "public key" can be a little misleading, as this information should not be shared with untrusted parties if you care about financial privacy. As Belcher has suggested, it may be helpful to refer to the xPub as a "[view key](https://twitter.com/chris_belcher_/status/1299281870757257218)" from a UX perspective, similar (though not technically equivalent) to how Zcash's "[viewing keys](https://z.cash/technology/)" are explicitly designed to expose transaction details that are otherwise obscured.
{: style="text-align: justify;"}

If you use a light wallet and [aren't connecting to your own node](https://samouraiwallet.com/dojo), chances are that you have shared your x/y/zPub with whoever operates that wallet service. This presents a serious privacy trade-off, though this design decision is not necessarily malicious. In order to display your address and / or account balance, your wallet needs to query a blockchain explorer to discover the coins it holds, and having the xPub is a convenient way to find out which addresses have been used.
{: style="text-align: justify;"}

In their [support documentation](https://support.ledger.com/hc/en-us/articles/360011069619-Extended-public-key), **Ledger** actually claims that "Ledger Live stores the xpub locally on your computer when you add the account. Your xpub is never sent to Ledger's servers. It is encrypted by your Ledger Live password if you have set one." In practice, this method would offer a minor but nonetheless important privacy advantage: if a Ledger user were to switch to using their own node, instead of continuing to rely on Ledger's node, Ledger the company would only be aware of the relationship between addresses that the user queried the balance for *prior* to migrating to their own node. They would not see the association with newly generated addresses going into the future (assuming the user avoids linking them through spending patterns).
{: style="text-align: justify;"}
